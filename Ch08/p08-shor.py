...
from projectq.backends import Simulator, ResourceCounter, IBMBackend
...
from projectq.libs.math import (AddConstant,
                                AddConstantModN,SubConstantModN,
...
def run_shor(eng, N, a, verbose=False):
    ...
    for k in range(2 * n):
        ...
        #mul_by_constant_modN(eng, current_a, N, x)
        ...
        if verbose:
            print(".".format(measurements[k]), end="")
            sys.stdout.flush()
...
# http://projectq.readthedocs.io/en/latest/examples.html#shor-s-algorithm-for-factoring
...
if __name__ == "__main__":
    ...
    backend = Simulator()
    eng = MainEngine(backend, compilerengines)
    ...
    print("\nFactoring N = {}: ".format(N), end="")

    # choose a base at random:
    a = int(random.random()*N)
    if not gcd(a, N) == 1:
        print("\n\noops, we were lucky: Chose non relative prime"
              " by accident :)")
        print("\tFactor: {}".format(gcd(a, N)))
    else:
        ...
        if f1 * f2 == N and f1 > 1 and f2 > 1:
            print("\n\nFactors found :-) : {} * {} = {}".format(f1, f2, N))
        else:
            print("\n\nBad luck: Found {} and {}".format(f1,f2))
